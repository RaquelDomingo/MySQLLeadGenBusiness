...
SELECT DATE_FORMAT(billing.charge_datetime, '%M') AS 'month', SUM(amount) as total revenue
FROM billing
...
SELECT billing.client_id, SU(billing.amount) as total_revenue
FROM billing
...
SELECT sites.client_id, DATE_FORMAT(sites.created_datetime, '%M') AS month_created, 
  DATE_FORMAT(sites.created_datetime, '%Y') AS year_created, COUNT(sites.site_id) as num_sites
FROM sites
...
-- What query would you run to get a list of client names and the total # leads
...
-- What query would you run to get a list of client names and the toal # of leads
-- we'v generated for each client each month between months 1 - 6 of Year 2011?
SELECT CONCAT(clients.first_name, '', clients.last_name) AS client_name, COUNT(leads.leads_id) AS num_leads, DATE_FORMAT(registered_datetime, '%M') AS 'month' 
FROM clients
  LEFT JOIN sites ON clients.client_id = sites.client_id
  JOIN leads ON sites.site_id = leads.site_id
WHERE leads.registered_datetime BETWEEN '2011-01-01' AND '2011-06-30'
...
SELECT CONCAT (clients.first_name, '', clients.last_name) AS client_name, sites.domain, COUNT(leads.leads_id) AS num_leads
FROM clients
  JOIN sites ON clients.client_id = sites.client_id
  LEFT JOIN leads ON sites.site_id = leads.site_id
GROUP BY clients.client_id, sites.domain_name;

-- Write a single query that retrieves total revenue colelcted from each client for each month of the year. Order by client id.
...
FROM clients
  LEFT JOIN billing ON clients.client_id = billing.clients_id
